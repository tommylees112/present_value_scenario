...
class ProbabilisticCareerAnalysis:
    ...
    def plot_cumprob_VaR(self, metric: str, confidence_level: float = 0.95):
        """
        Plot cumulative probability distribution and Value at Risk for a given metric

        Parameters:
        -----------
        metric: str
            The metric to analyze (e.g., 'Total Nominal Cost', 'Break-even Year')
        ...
        sorted_pairs = sorted(zip(values, probs))
        sorted_values, sorted_probs = zip(*sorted_pairs)
        ...
        ax.step(
            sorted_values, cum_probs, "b-", label="Cumulative Probability", where="post"
        )

        # Find VaR
        var_value = None
        for value, cum_prob in zip(sorted_values, cum_probs):
            if cum_prob >= 1 - confidence_level:
                var_value = value
                break
        ...
        if var_value is not None:
            ...
            # Add VaR annotation
            ax.annotate(
                f"VaR: £{var_value/1000:.0f}k",
                xy=(var_value, 1 - confidence_level),
                xytext=(10, 10),
        ...
        # Formatting
        ax.set_title(f"Cumulative Probability Distribution and VaR for {metric}")
